...
class Centro_Asistencia_Manager(models.Manager):
	...
	def asignado_para_mi(self, user):
		...
		return self.model.objects.filter(estado=True, personal_operativo__usuario=user).exclude(personal_operativo__grupo__name='JEFE DEPARTAMENTO')
	...
	def administrado_por_mi(self, user):
		...
		return self.model.objects.filter(estado=True, personal_operativo__usuario=user, personal_operativo__grupo__name='JEFE DEPARTAMENTO')
		# return self.model.objects.filter(usuarios__dni=user)
	...
	def centro_administrado_por_mi(self, user, centro):
		...
		return self.model.objects.filter(estado=True, personal_operativo__usuario=user, personal_operativo__grupo__name='JEFE DEPARTAMENTO', pk=centro)

	def acesorado_por_mi(self, user):		
		return self.model.objects.filter(estado=True, personal_operativo__usuario=user, personal_operativo__grupo__name='ASESOR TECNICO')
